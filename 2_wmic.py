...
def get_system_processes():
    ...
    if platform.system() == "Windows":
        # 在Windows系统中，使用wmic命令获取进程列表
        try:
            process_output = subprocess.check_output(['wmic', 'process', 'list', 'brief'], text=True).strip()
            # wmic输出的格式是每行一个进程名，我们通过split('\n')来分割它们
            processes = process_output.split('\n')
            # 获取进程的可执行文件名，wmic输出的第一列是进程名
            process_names = [process.split(None, 2)[1] for process in processes if process]
...
# 初始化一个空的字符串，用于累积所有的杀毒软件进程信息
        ...
        if detected_antiviruses:
            ...
            print("杀毒软件进程已记录到scan_ok.txt文件。")
        else:
            print("未检测到已知的杀毒软件进程。")
            write_to_file('scan_ok.txt', "未检测到已知的杀毒软件进程。")
